...
**ID 와 코드의 구분**
앞에서도 언급했지만, 개별 개체를 구분할 때는 아이디 `Id` 를, 미리 정의된 특정 범주값을 나타낼 때는 코드 `Cd` 를 필드의 접미어로 사용했다. 예를 들어 특정 아이템에 대해 `ItemId` 는 그 아이템 개체를 식별하기 위한 값이고,  `ItemCd` 는 그 아이템이 어떤 종류인지에 분류하기 위한 값이다. `Id` 는 임의값으로 유니크하면 되고, `Cd` 는 미리 정의된 값으로 문서화된 설명이 있어야 한다.

**맵 코드와 좌표**
게임내 특정 지역에서 발생하는 이벤트를 위해 맵 코드와 위치 좌표 필드를 포함하였다. 예제에서는 계정이나 시스템 측면에서 발생하는 로그도 함께 다루기 위해 일단 옵션으로 설정하였으나, 가능한 경우는 꼭 기록하는 것이 분석에 도움이 된다. 

**랜덤 ID 이용**
하나의 사건에서 여러 로그 이벤트가 발생하는 경우가 있다. 예를 들어 거래소에서 아이템을 구매하는 경우 아이템은 들어오고 돈은 빠져나가야 한다. 로그 측면에서는 아이템 증가 로그와 돈 감소 로그가 함께 발생하는 것이다. 이런 경우 분석을 위해서는 한 사건의 연관 로그들을 찾아볼 수 있어야 하는데, 필자는 랜덤 ID 방식을 추천한다. 랜덤 ID 는 사건 발생 시점에서 랜덤 정수 하나를 만들고, 그것을 연관된 로그들의 `LinkRd` 필드에 기입하는 방식이다. 랜덤한 정수는 왠만해서는 일치하기 힘들기에 비슷한 시간대에 발생한 연관 로그들을 찾기에는 충분한 식별력을 가진다.
