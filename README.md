...
지금까지 필드를 만들 때 사용했던 타입 정보만으로는 실제 로그의 특성을 충분히 반영하지 못할 수 있다. 예를 들어 `ServerNo` 필드의 경우, 단순히 정수가 아닌 1 이상의 정수가 와야할 것이다. 이렇게 필드의 값에 관해 더 세분화된 정보를 지정하는 것을 **필드값을 제약(restrict)** 한다고 하겠다.

> 필드값의 제약을 미리 해두면 뒤에 나올 로그 검증 및 더미 로그 생성에 요긴하게 사용된다.

필드값을 제약하기 위해서는 `fields` 항목에서 지금까지 사용하던 리스트 요소 대신 오브젝트 방식으로 기술해야 한다.
