...
- [LogLab](#loglab)
  - [LogLab(로그랩) 은 다양한 서비스를 위한 로그를 설계하고 활용하기 위한 툴이다.](#loglab로그랩-은-다양한-서비스를-위한-로그를-설계하고-활용하기-위한-툴이다)
  - [로그랩 소개](#로그랩-소개)
    - [기능](#기능)
    - [로그의 형식](#로그의-형식)
    - [대상 사용자](#대상-사용자)
...
여기에는 다음과 같은 몇 가지 문제를 제기할 수 있겠다:

- 왜 자유로운 형식의 텍스트 로그가 아닌가?
완전 비정형 텍스트 로그는 작성하는 측에서는 편하지만 파싱 및 관리가 힘들기에 원하는 정보를 추출하는데 한계가 있다.

- 왜 CSV 가 아닌 JSON 형식인가?
JSON 은 언듯 키+값의 형식으로 언듯 장황(Verbose)해 보이지만, 로그 구조 변경이 용이한 장점이 있다. 반면, CSV 는 서비스 중 필드의 수가 자주 변할 때 대응하기가 까다롭다. 또한 JSON 의 키는 반복적으로 나타나기에 압축시 효율이 좋다 .

- 왜 DB에 남기지 않는가?
DB에 남기는 로그는 잘 정규화되어 정보 활용측면에서 뛰어나지만 다음과 같은 문제점이 있다.
  - 로그 형식 변경이 까다롭다.
  - 파일 로그는 별도의 서버나 장비없이도 손쉽게 날길 수 있고, 관리 부담이 적다.
...
처음으로 나오는 `domain` 최상단 요소는 랩 파일의 **도메인(Domain) 정보** 를 기술하는데 사용한다. 도메인 요소는 랩 파일이 어떤 서비스를 위한 것인가에 대한 정보를 담는데, 모든 랩 파일에 꼭 있어야 하는 **필수 요소** 이다.

위 예처럼 `domain` 항목 아래 도메인 이름 (`name`) 및 설명 (`desc`) 을 입력하면 되는데, 도메인 이름은 나중에 해당 랩 파일을 식별하는 용도로 사용되기에, 알파벳 소문자와 숫자, 그리고 밑줄 문자 `_` 만을 사용해 **식별 가능한 범위에서 간략하게 기술**한다.

> 관용적으로 랩 파일의 이름과 도메인 이름을 같게 하는 것을 추천한다.

로그랩에서는 로깅의 대상이 되는 각 사건을 **이벤트(event)** 라고 하는데, 랩 파알의 `events` 최상단 요소에 하나 이상의 이벤트 요소를 기술할 수 있다. 예제에서는 계정 로그인 이벤트를 위한 `Login` 요소를 만들었다. 그 아래의 `desc` 요소는 이벤트에 대한 설명을 위한 것이다. 필수 요소는 아니지만 문서화를 위해서는 꼭 필요하다.
...
Options:
  ...
  -k, --keep-text    긴 문자열 필링 않음
  --help             Show this message and exit.
```

`show` 는 랩파일을 인자로 필요로 하는 것을 알 수 있다. 이제 아래와 같이 실행한다.
...
> 앞의 `Login` 과 `DateTime` 에서 알 수 있듯, 로그랩의 이벤트 및 필드 이름은 단어의 시작을 대문자로 (Pascal Case) 하고, 타입 이름은 소문자로 시작 후 합성어의 첫 단어를 대문자로 (Camel Case) 하는 것을 권장한다.
...
사용하는 에디터가 VS Code 처럼 JSON 스키마를 지원한다면, 아래와 같은 가이드를 볼 수 있을 것이다.

![스키마 가이드](image/guide.png)

또한, 문맥에 맞지 않는 요소나 틀린 철자등을 찾아주기에 편리하다.
## 필드의 추가
...
필드는 이벤트 요소 아래 `fields` 리스트에 기술하는데, 각 필드는 3 개 항목을 가지는 리스트로, 아래와 같은 형식이다.
...
세 번째 항목인 필드 설명은 필수가 아니나 문서화를 위해서는 꼭 필요하다.
...
그런데 로그인, 로그아웃 이벤트 모두 `AcntId` 필드을 가지고 있는 것이 보인다. 앞으로 계정에 관한 다른 이벤트를 만든다면 거기에도 모두 이 필드를 만들어 주어야 할 것이다. 반복 작업을 방지할 수 없을까?

## 믹스인을 활용한 리팩토링

**믹스인(mixin)** 은 다른 요소의 필드를 가져다 쓰는 방법이다. 믹스인을 활용하면 다양한 이벤트에서 공통적으로 필요한 필드를 재활용할 수 있다.
...
> 이벤트는 베이스 뿐만 아니라 다른 이벤트도 믹스인할 수 있다. 이 경우 `events.이벤트_이름` 형식으로 지정하면 된다. 그러나, 베이스는 이벤트를 믹스인할 수 없다.
...
두 이벤트에 `fields` 항목이 없지만 `Account` 베이스의 필드를 가져와 같은 결과가 나오는 것을 알 수 있다. 이런 식으로 서로 다른 요소에서 공통 요소를 추출해 내는 것을 **리팩토링(refactoring)** 이라고 한다.

> 리팩토링은 왜 필요할까? 예를 들어 계정에 관한 필드를 가지는 로그 이벤트가 100 개 있다고 하자. 어느날 계정 정보에 필드 하나가 추가되어야 한다는 요청이 들어오면, 리팩토링이 되지 않은 경우 100 개나 되는 이벤트를 모두 수정해야 할 것이다. 미리 계정 관련 베이스를 만들어 리팩토링 해두었다면, 단 한 번의 수정으로 100 개 이벤트 모두에 추가 필드를 적용할 수 있을 것이다..

참고로, `show` 명령의 결과에 베이스는 출력되지 않는다. 베이스는 참조되어 사용되어질 뿐, 그 자체로 이벤트는 아니기 때문이다.

## 게임관련 이벤트와 필드의 추가

이제 기본적인 랩 파일 작성 방법을 알게 되었다. 지금까지 배운 것을 활용하여 실제 게임에서 발생할 수 있는 다양한 이벤트를 추가해보자.
...
### 믹스인의 처리 순서
 ...
 믹스인은 `mixin` 리스트에 등장하는 순서대로 수행되며, 앞 항목과 뒤 항목에 일치하는 필드가 있다면 뒤의 것으로 덮어쓰게 된다. 이것을 이용하면 특정 필드의 순서를 조정하거나 재정의 할 수 있다. 예를 들어 위 예에서 `ServerNo` 필드가 `AcntId` 보다 먼저 나오게 하고 싶다면 `mixin` 리스트의 항목 순서를 다음과 같이 바꿔주면 된다.
...
베이스는 다른 베이스를 믹스인 할 수 있다. 예제의 서비스에서 계정에 관한 이벤트가 항상 서버 단위로 일어난다면, 아래와 같이`Account` 베이스에 `Server` 베이스를 포함할 수 있다.
...
> 믹스인을 할 요소의 `bases` 내 기술 순서는 중요하지 않다. 위의 경우 `Account` 가 `Server` 앞에 오더라도 문제가 없다.

### 옵션 필드

지금까지 등장한 모든 필드들은 기본적으로 로그 이벤트에 반드시 나와야 하는 (required) 필드들이었다. 만약 나올 수도 있고 안 나와도 괜찮은 필드가 있다면, **옵션(option)** 으로 만들 수 있다. 그것은 `fields` 리스트의 항목별 4번째 항목에 `true` 또는 `false` 를 지정하여 만들 수 있다 (`false` 는 기본값이기에 굳이 기술할 필요가 없겠다).
...
보통 실제 플레이를 하는 것은 계정이 자신이 아니라 계정에 속한 캐릭터이다. 이에 캐릭터 관련 이벤트를 추가해보겠다.

한 계정은 하나 이상의 캐릭터를 소유하고 선택하여 플레이할 수 있기에, 다음과 같이 `Character` 베이스를 추가한다.
...
`Character` 베이스는 `Account` 의 모든 필드를 이용하게 된다. 다음은 `Character` 베이스를 이용해, 캐릭터의 로그인/아웃 이벤트를 추가한다.
...
믹스인한 베이스의 정보, 즉 계정 및 캐릭터, 지도상의 위치, 몬스터 개체에 관한 정보들이 잘 결합된 것을 확인할 수 있다. 이런 식으로 베이스를 잘 설계하고 그것을 믹스인하는 것 만으로, 다양한 로그 이벤트를 쉽게 만들 수 있다.
...
지금까지는 필드를 만들 때 기본 타입만을 사용했는데, 실제 특성을 충분히 반영하지 못할 수 있다. 예를 들어 `Server` 베이스의 `ServerNo` 필드의 경우, 단순히 정수가 아닌 1 이상의 정수가 와야 할 것이다. 이렇게 필드의 값에 더 자세한 제한을 거는 것을 **필드값을 제약(restrict)** 한다고 하겠다.

> 필드값의 제약은 로그 설계 측면에서 꼭 필요한 것은 아니지만, 뒤에 나올 로그 검증 및 더미 로그 생성 기능을 이용할 때 유용하다.

### 서버 번호에 제약 걸기

필드값을 제약하기 위해서는 `fields` 항목에서 지금까지 사용하던 리스트 대신 `키:값` 형식으로 기술해야 한다. 예로 `ServerNo` 필드를 제약해보자.
...
기존 리스트 `["ServerNo", "integer", "서버 번호"]` 대신 `name`, `desc`, `type` 키를 사용하는 식으로 바뀌었다. 끝에 추가된 `minimum` 부분은 **제약문** 으로 `ServerNo`의 값을 1 이상으로 제약하고 있다.
...
랩 파일에 `types` 최상단 요소를 만들고, 그 아래 커스텀 타입을 정의한다. 커스텀 타입의 설명은 실제 출력되지는 않으나 문서화 측면에서 꼭 기입하도록 하자. 예에서는 `unsigned` 라는 커스텀 타입을 만들고 0 이상의 정수가 되도록 하였다.

> 랩 파일에서 최상단 요소의 등장 순서는 중요하지 않으나, 관례적으로 `domain`, `types`, `bases`, `events` 순으로 기술하는 것을 추천한다.

이것을 필드에서 이용할 때는 `types.타입_이름` 형식의 경로로 지정한다. 아래를 참고하자.
...
> 기억해야 할 것은, **커스텀 타입을 이용하는 필드에는 추가적인 제약을 걸 수 없다** 는 점이다. 따라서, 필드 리스트에 `types.*` 로 커스텀 타입을 지정하는 것은 리스트 형식에서만 가능하다.
...
이제 출력이 바뀌었다. 먼저 이벤트에 앞서 정의된 커스텀 타입을 보여주고, 이벤트에서 커스텀 타입을 이용하는 필드는 기본 타입으로 변환하지 않고 커스텀 타입 경로를 그대로 보여준다.
...
> `enum` 은 `string` 뿐만 아니라, `integer` 와 `number` 타입에 대해서도 사용할 수 있다.

나열형 항목의 값에 대해 구체적인 설명을 붙여야 하는 경우도 있다. `Item` 베이스의 `ItemCd` 필드에 나열을 이용해 등장할 수 있는 값을 제한하고 설명도 붙이는 경우를 생각해 보자.
...
기존 제약을 `enum` 으로 대체할 것이기에, 기존 `types.unsigned` 대신 `integer` 로 타입을 변경하였다. 이어서 각 값에 대한 설명을 아래와 같이 추가한다.
...
`1`, `2` 같은 숫자 대신 `[나열값, 나열값_설명]` 형식의 리스트를 사용한다. `show` 을 실행해보면 `ItemCd` 필드의 제약 컬럼에 각 나열값의 설명이 나오는 것을 알 수 있다.
...
로그를 설계한 후 실제 서비스에서는 설계에 맞게 출력하도록 구현이 필요하다. 구현하는 측에서는 잘 구현되었는지 확인을 하고 싶은 경우가 많다. 로그랩을 사용하면 설계된 로그의 정보를 이용하여 실제 로그를 검증할 수 있다. `loglab` 의 `verify` 명령으로 검증할 수 있는데, 먼저 도움말을 살펴보자.
...
다음으로 검증을 위한 로그 스키마가 필요하다. 다음과 같은 `schema` 명령으로 지금까지 작성한 랩 파일에서 로그 검증용 스키마를 만들수 있다.
...
JSON 스키마 형식으로 두 가지 파일이 생성되는데, 하나는 **로그 스키마 (.log.schema.json)** 이며 다른 하나는 **플로우 스키마 (.flow.schema.json)** 이다. 로그 스키마는 실제 로그를 검증하는데 사용되고, 플로우 스키마는 더미 로그를 설계할 때 사용되는데 나중에 다룰 것이다.

이제 `verify` 명령으로 검증을 수행한다.
...
> 하나 이상의 외부 랩 파일을 가져올 수 있으며, 겹치는 것은 나중에 나오는 것이 우선한다.
...
`show` 를 해보면 `acme.lab.json` 에서 정의된 `Login` 이벤트 그대로 나온다.
...
`Login` 에 맞도록 설명을 추가하고, `Platform` 필드도 PC 온라인 서비스에 맞게 변경해보자.
...
```js
{
  ...
  "events": {
    ...
    "ServerMemory": {
      ...
      "fields": [
        ["AvailMemory", "acme.type.unsigned", "가용 메모리"]
      ]
    }
  }
}
...
이런 식으로 공용 랩파일을 만들고 그것에 정의된 타입, 베이스, 이벤트를 잘 활용하면 조직의 표준을 따르면서도 서비스 별 특성에 맞는 확장이 가능할 것이다.


> 외부 랩 파일을 활용할 때는 먼저 어떤 타입과 베이스가 정의되어 있는지 잘 파악하고, 가급적 활용하도록 하자. 추후 변경이나, 로그 처리 시 도움이 될 것이다.


### 설명 찾기의 이해

앞에서 설명은 필수가 아니나, 문서화를 위해 꼭 필요하다고 했다. 이 말은 `show` 나 `html` 같은 명령을 통해 문서를 출력할 때는 각 요소에 대한 설명을 최종적으로 찾을 수 있어야 한다는 뜻이다.

- 로그랩은 먼저 각 베이스/이벤트에 설명이 없지만 믹스인은 있는 경우, 믹스인 되는 요소의 설명을 순서대로 이용한다.
- 설명과 믹스인이 모두 없거나, 믹스인 되는 요소에도 설명이 없다면 아래와 같은 에러가 발생한다.
  - ` Exception: Can not resolve description for 'Login'.`

예를 들어 `boo.lab.json` 의 `Login` 에서 `desc` 요소를 생략하고 `show` 를 해보면,
...
`acme.event.Login` 의 설명을 대신 출력하는 것을 알 수 있다.
...
로그랩을 통해 설계/운용 되는 로그들은 랩 파일의 신택스로 모든 것이 표현된다. 즉, 버전별 차이를 알기 위해서는 두 버전간 랩 파일의 차이를 diff 등의 툴로 비교해보면 간단히 알 수 있다. 로그 변경 이력을 수작업으로 작성하고 공유할 필요가 사라지는 것이다.
