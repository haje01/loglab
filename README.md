...
> 리팩토링은 왜 필요할까? 예를 들어 계정에 관한 필드를 가지는 로그 이벤트가 50 개 있다고 하자. 어느날 계정 정보에 필드 하나가 추가되어야 한다는 요청이 들어오면, 리팩토링이 되지 않은 경우 50 개나 되는 이벤트를 모두 수정해야 할 것이다. 미리 계정 관련 베이스를 만들어 리팩토링 해두었다면, 단 한 번의 수정으로 50 개 이벤트 모두에 추가 필드를 적용할 수 있을 것이다..
