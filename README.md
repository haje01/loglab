...
하나의 사건에서 여러 로그 이벤트가 발생하는 경우가 있다. 예를 들어 거래소에서 아이템을 구매하는 경우 아이템은 들어오고 돈은 빠져나가야 한다. 로그 측면에서는 아이템 증가 로그와 돈 감소 로그가 함께 발생하는 것이다. 이런 경우 분석을 위해서는 한 사건의 연관 로그들을 찾아볼 수 있어야 하는데, 필자는 **랜덤 ID** 방식을 추천한다. 랜덤 ID 는 사건 발생 시점에서 랜덤 정수 하나를 만들고, 그것을 연관된 로그들의 같은 필드 (예제에서는 `LinkRd`) 필드에 기입하는 방식이다. 랜덤한 정수는 왠만해서는 일치하기 힘들기에 비슷한 시간대에 발생한 연관 로그들을 찾기에는 충분한 식별력을 가진다.

예제가 로그랩의 활용에 도움이 되기를 바란다.
