...
로그랩에서는 로깅의 대상이 되는 각 사건을 **이벤트(event)** 라고 하는데, 랩 파알의 `events` 최상단 요소에 하나 이상의 이벤트 요소를 기술할 수 있다. 예제에서는 계정 로그인 이벤트를 위한 `Login` 요소를 만들었다. 그 아래의 `desc` 요소는 이벤트에 대한 설명을 위한 것인데, 모든 이벤트에 필요한 필수 요소이다.
...
최상단 요소 `import` 는 가져올 외부 랩 파일 리스트를 가진다.

> 하나 이상의 외부 랩 파일을 가져올 수 있으며, 겹치는 것은 나중에 나오는 것이 우선한다.

외부 랩 파일에서 선언한 모든 커스텀 타입, 베이스와 이벤트를 이용할 수 있다.
