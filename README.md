...
`Event` 필드에 이벤트 이름을, `DateTime` 에 이벤트 발생 시간을 기입한다. 나머지 필드는 랩 파일에서 정의한 것이다.

> 파일 내용에 맞는 확장자는 `.jsonl` 이겠으나, 로그 내용을 확인하기 좋도록 `.txt` 확장자를 사용하였다.

다음으로 검증을 위한 로그 스키마가 필요하다. 다음과 같은 `schema` 명령으로 지금까지 작성한 랩 파일에서 로그 검증용 JSON 스키마를 만들수 있다.
