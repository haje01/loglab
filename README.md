...
랩 파일에 첫 번째로 나오는 `domain` 요소는 랩 파일의 **도메인(Domain) 정보**를 기술하는데 사용한다. 도메인 요소는 랩 파일이 어떤 서비스를 위한 것인가에 대한 정보를 담는데, 모든 랩 파일에 꼭 있어야 하는 **필수 요소** 이다.
...
로그랩에서는 로깅의 대상이 되는 각 사건을 **이벤트(event)** 라고 하는데, 위 랩  파일에서 두 번째로 나오는 `events` 요소에 하나 이상ㅇ의 이벤트 요소를 기술할 수 있다. 예에서 계정 로그인 이벤트를 위한 `Login` 요소를 만들었다. 그 아래의 `desc` 요소는 이벤트에 대한 설명을 위한 것인데, 모든 이벤트에 꼭 필요한 필수 요소이다.
