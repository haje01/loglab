...
지금까지 필드를 만들 때 사용했던 타입 정보만으로는 실제 로그의 특성을 충분히 반영하지 못할 수 있다. 예를 들어 `ServerNo` 필드의 경우, 단순히 정수가 아닌 1 이상의 정수가 와야 할 것이다. 이렇게 필드의 값에 관해 더 세분화된 정보를 지정하는 것을 **필드값을 제약(restrict)** 한다고 하겠다.
...
기존 리스트 요소 `["ServerNo", "integer", "서버 번호"]` 에서 오브젝트 요소로 바뀌었다.
