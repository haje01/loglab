...
앞에서 설명한 것 처럼, 커스텀 타입은 `fields` 의 리스트 형 항목에서만 사용할 수 있기에 기존 `types.id` 대신 `integer` 로 타입을 변경하였다. 여기에서 설명을 추가하기 위해 아래와 같이 수정할 수 있다.
...
`doc` 을 실행해보면 `ItemTypeId` 필드의 제약 컬럼에 각 나열값의 설명이 나오는 것을 알 수 있다.
