...
분석의 대상이 되는 주요 이벤트들 뿐만 아니라, 개발자에게 도움이 되는 디버깅 관련 정보도 로그로 남길 필요가 있다. 디버그 로그를 별도의 파일에 저장하는 것도 가능하지만, 이벤트 로그와 디버그 로그를 하나의 타임라인에 놓고 분석하는 것이 디버깅에 더 용이할 것이다. 이에 디버그 로그 `Debug` 를 정의하고 메시지 `Msg` 필드에 자유로운 텍스트를 남기도록 하였다.
