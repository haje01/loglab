...
- LogLab은 메타파일을 사용해 다음과 같은 일을 할 수 있다.
    - JSON, csv, tsv 형식의 샘플 로그 생성
    - 로그 문서 생성
    - JSON 스키마 생성
...
- 샘플 로그에서 출력된 이벤트 일시와 이름도 사실 속성이며, 이것은 LogLab에 의해 기본으로 추가되었다.
...
서버의 번호 속성에 - 값이 나오고 있어 좀 이상한다. 속성 값의 범위를 지정하면 로그의 엄격한 검증이 가능하고, 샘플 생성도 자연스러워진다. 위에서 사용한 **리스트형 속성 선언**은 간략해서 보기가 좋지만, 기술 내용에 한계가 있다. 범위 지정 등 상세한 명세를 지정하기 위해서는 **객체형 속성 선언**이 필요하다.
...
```
객체형 속성 선언은 기본적으로 [JSON Schema 스펙](https://json-schema.org/understanding-json-schema/reference/index.html)에 따르니, 자세한 내용은 링크의 문서를 참고하기 바란다.
...
```
온라인 게임에 맞게 `Platform`의 `enum`을 다시 정의하였다.
