...
지금가지 소개한 방법으로 다양한 이벤트를 정의하고, 외부 랩 파일까지 쓰다보면 한 눈에 구조를 파악하기가 점점 힘들어진다. 이럴 때는 `show` 에 필터를 걸어서 보면 편리하다.

필터는 `show` 명령의 `-f` 또는 `--filter` 옵션을 이용해 찾는 타입/베이스/이벤트 이름의 패턴을 준다. 예를 들어 캐릭터 관련 이벤트만 보고 싶다면 아래와 같이 한다.
