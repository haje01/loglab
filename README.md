...
문제가 되는 것은 로그랩에서 설명을 위해 자동으로 추가되는 메시지들 (`이벤트 시간`, `~이상`, `~미만` 등) 이 한국어로 나오는 것인데, 로그랩은 메시지 언어를 `언어_지역` 형식의 로케일로 선택하는 기능을 제공한다 (현재는 영어 `en_US` 와 중국어 `zh_CN` 가 준비되어 있다).
...
아래와 같이 `show` 명령에서 `-l` 또는 `--lang` 옵션을 통해 메시지 언어를 선택해보자.
...
지금까지 작성한 랩 파일을 사용해 설명이 한국어로 나오지만, 로그랩에서 추가한 설명은 지정한 언어로 나오는 것을 확인할 수 있다. 앞에서 설명한 `html` 명령도 같은 식으로 동작한다.
