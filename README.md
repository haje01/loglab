...
이벤트 자체의 설명이 없는 경우 믹스인된 `Account` 의 설명 이 그대로 출력되는 것을 알 수 있는데, 이것은 **설명 찾기** 기능의 결과이다.
...
> 외부 랩 파일을 활용할 때는 먼저 어떤 타입과 베이스가 정의되어 있는지 잘 살펴보고 활용하도록 하자. 추후 변경이나, 로그 처리 시 도움이 될 것이다.
