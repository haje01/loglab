...
> **설명 찾기**
>
> 앞에서 설명은 필수가 아니나, 문서화를 위해 꼭 필요하다고 했다. 이 말은 `show` 나 앞으로 설명할 `html` 같은 명령을 통해 문서를 출력할 때는 각 요소에 대한 설명을 최종적으로 찾을 수 있어야 한다는 뜻이다. 요소 자체의 설명이 없는 경우 아래와 같이 설명 찾기를 시도한다.
>
...
이런 식으로 공용 랩 파일을 만들고 그것에 정의된 타입, 베이스, 이벤트를 활용하면 조직의 표준을 따르면서도 서비스 별 특성에 맞는 확장이 가능할 것이다.
...
특정 서비스를 장기간 운용하다보면 버전별 로그 변경 내용을 문서화하고 공유하는 것도 큰 일이다. 로그랩을 통해 설계/운용되는 로그는 텍스트 형식인 랩 파일 안에 로그 구조의 모든 것이 표현되기에, 텍스트 파일의 차이를 비교하는 `diff` 등의 툴을 이용하면 버전별 로그 구조의 차이를 간단히 표현할 수 있다. 로그 변경 이력을 수작업으로 기록할 필요가 사라지는 것이다.


### MMORPG 위한 예제

로그랩을 본격적으로 큰 프로젝트에 사용할 때 참고가 될 만한 예제가 있다. MMORPG 게임을 위해 게임의 주요 이벤트들을 로그랩으로 기술한 것이다.
...
하나의 사건에서 여러 로그 이벤트가 발생하는 경우가 있다. 예를 들어 거래소에서 아이템을 구매하는 경우 아이템은 들어오고 돈은 빠져나가야 한다. 로그 측면에서는 아이템 증가 로그와 돈 감소 로그가 함께 발생하는 것이다. 이런 경우 분석을 위해서는 한 사건의 연관 로그들을 찾아볼 수 있어야 하는데, 필자는 **랜덤 ID** 방식을 추천한다. 랜덤 ID 는 사건 발생 시점에서 랜덤 정수 하나를 만들고, 그것을 연관된 로그들의 같은 필드 (예제에서는 `LinkRd`) 에 기입하는 방식이다. 랜덤한 정수는 웬만해서는 일치하기 힘들기에, 비슷한 시간대에 발생한 연관 로그들을 찾기에는 충분한 식별력을 가진다.

**캐릭터 싱크 로그**

싱크 로그는 서버에서 정기적 (예: 5 분) 으로 캐릭터의 정보를 로그로 출력하는 것이다. 예제에서는 캐릭터 상태 `CharSync` 와 캐릭터 머니 상태 `CharMoneySync` 이벤트로 구성하였다. 머니는 종류에 따라 다양할 수 있기에 별도 이벤트로 분리하였고 `LinkRd` 로 연결해서 보도록 하였다. 싱크 로그는 게임내 이상현상이나 어뷰징 탐지에 활용될 수 있다.

**디버그 로그**
분석의 대상이 되는 주요 이벤트들 뿐만 아니라, 개발자에게 도움이 되는 디버깅 관련 정보도 로그로 남길 필요가 있다. 디버그용 로그를 별도의 파일에 저장하는 것도 가능하지만, 이벤트 로그와 디버그 로그를 하나의 타임라인에 놓고 분석하는 것이 디버깅에 더 용이할 것이다. 이에 디버그 로그 `Debug` 를 정의하고 메시지 `Msg` 필드에 자유로운 텍스트를 남기도록 하였다.
