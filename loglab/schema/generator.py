...
from .models import DomainInfo, EventSchema
...
class LogSchemaGenerator:
    ...
    def __init__(
        self, property_builder: PropertyBuilder = None, config: SchemaConfig = None
    ):
    ...
    def generate_schema(self, lab_data: Dict[str, Any]) -> str:
        ...
        domain_info = DomainInfo(
            name=domain_model.domain.name, description=domain_model.domain.desc
        )
    ...
    def _build_event_schema(self, event_name: str, event_data: AttrDict) -> EventSchema:
        ...
        """
        # 속성들 생성 (문자열 리스트로)
        property_strings = self.property_builder.build_properties_from_fields(
            event_data.fields
        )
        ...
        # 필수 필드들 추출
        required_fields = self.property_builder.extract_required_fields(
            event_data.fields
        )
    ...
    def _assemble_schema(
        self, domain_info: DomainInfo, event_schemas: List[EventSchema]
    ) -> str:
        ...
        for event_schema in event_schemas:
            ...
            # 필수 필드들을 조합
            required_str = ", ".join(
                [f'"{field}"' for field in event_schema.required_fields]
            )
