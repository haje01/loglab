...
from .interfaces import FileLoader, JsonValidator, ErrorHandler, ValidationResult
            ...
            if 'import' in lab_data:
                ...
                for imp in lab_data['import']:
                    ...
                    try:
                        ...
                        result = self._recursive_validate(import_data, schema_data, import_path)
                        if not result.success:
                            # import된 파일에서 에러가 발생하면 컨텍스트 추가
                            result.errors = [f"In imported file {import_path}: {error}" for error in result.errors]
                            return result
