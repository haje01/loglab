...
from .interfaces import ErrorHandler, ValidationResult


class LogFileValidator:
    """로그 파일 검증기 클래스."""

    def __init__(self, error_handler: ErrorHandler = None, config: SchemaConfig = None):
        ...
        try:
            ...
            if not schema_data:
                return ValidationResult(
                    success=False, errors=["Failed to load schema file"]
                )
    ...
    def _validate_log_entries(
        self, log_path: str, event_schemas: Dict[str, Dict[str, Any]]
    ) -> ValidationResult:
        """로그 엔트리들을 검증.
    ...
    def _parse_log_file(
        ...
        try:
                ...
                for line_no, line in enumerate(f):
                    try:
                        ...
                        return ValidationResult(
                            success=False,
                            errors=[
                                f"[Line: {line_no + 1}] 유효한 JSON 형식이 아닙니다: {e}"
                            ],
                        )

                    # 이벤트 타입 확인
                    if (
                        "Event" not in log_entry
                        or log_entry["Event"] not in event_schemas
                    ):
                        return ValidationResult(
                            success=False,
                            errors=[
                                f"[Line: {line_no + 1}] 스키마에서 이벤트를 찾을 수 없습니다: {line.strip()}"
                            ],
                        )
            ...
            return ValidationResult(
                success=False, errors=[f"Error reading log file: {e}"]
            )

    def _validate_by_events(
        self, log_data: Dict[str, Any], event_schemas: Dict[str, Dict[str, Any]]
    ) -> ValidationResult:
        """이벤트별로 로그 엔트리들을 검증.
        ...
        for event_name, logs in event_logs.items():
            ...
            try:
                ...
                return ValidationResult(
                    success=False,
                    errors=[
                        f"[Line: {line_no + 1}] {e.message}: {json.dumps(error_log, ensure_ascii=False)}"
                    ],
                )
