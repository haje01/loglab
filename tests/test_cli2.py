...
from loglab.cli import cli, html
from loglab.cli import object as object_cmd
from loglab.cli import schema, show, verify, version
...
class TestCLIBasics:
    ...
    @pytest.mark.parametrize(
        "command_name", ["show", "schema", "verify", "html", "object"]
    )
    ...
    def test_show_output_contains_expected_sections(self):
        ...
        with self.runner.isolated_filesystem():
            lab_content = {
                ...
                "events": {
                    "TestEvent": {
                        ...
                        "fields": [["DateTime", "datetime", "Event time"]],
                    }
                },
            }
            ...
            with patch("loglab.cli.handle_import"):
                ...
                # Domain 정보 확인
                assert any(
                    "Domain" in line and "testdomain" in line for line in output_lines
                )

                # Event 정보 확인
                assert any(
                    "Event" in line and "TestEvent" in line for line in output_lines
                )
...
class TestCommandBehaviorValidation:
    ...
    def test_html_command_output_generation(self):
        """html 명령어의 출력 생성 확인."""
        with self.runner.isolated_filesystem():
            lab_content = {"domain": {"name": "test", "desc": "Test Domain"}}

            with open("test.lab.json", "w") as f:
                json.dump(lab_content, f)

            with patch("loglab.cli.handle_import"), patch(
                "loglab.cli.html_from_labfile", return_value="<html>test</html>"
            ):
    ...
    def test_object_command_language_support(self, language):
        ...
        with self.runner.isolated_filesystem():
            ...
            with patch("loglab.cli.handle_import"), patch(
                "loglab.cli.object_from_labfile", return_value=f"// {language} code"
            ):
